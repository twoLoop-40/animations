...
function linearTransformation(
  ...
  return (value: number) => {
    return slope * (value - source.start) + target.start;
  };
}

export const airbnbPath =
...
export function makeVars(variants: Variants) {
  return {
    ...variants,
  };
}

export const isMainDiagonal = (i: number) => {
  return i === 1 || i === 4 ? true : false;
};
